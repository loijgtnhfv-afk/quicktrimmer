...
動画ファイルは**サーバーに一切送信されません**。読み込み・カット・書き出しはすべてあなたのブラウザ内（ffmpeg.wasm）で完結します。クリップに含まれるボイスチャット・ユーザー名・ゲーム内情報が外部に出ることはありません。

アクセス解析として Vercel Web Analytics / Speed Insights（匿名のページ表示数・表示速度の計測）のみ使用しています。動画の中身は計測対象外です。
